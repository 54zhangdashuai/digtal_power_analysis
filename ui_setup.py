...
class UiSetupMixin:
    """一个包含所有UI创建和布局方法的Mixin类"""

    def initUI(self):
        self.setWindowTitle('数字电源IDE - 理论与实践 v5.2 (优化版)  作者：张辰')
        self.setGeometry(80, 60, 1600, 940)
        # Assuming favicon.ico is in the same directory
        ...
        main_splitter.setSizes([800, 1300])
    ...
    def _create_left_panel(self):
        ...
        self.main_tabs.setMinimumHeight(0)
    ...
    def _create_data_analysis_tab(self):
        ...
        layout.addWidget(column_select_group)
    ...
    def _create_roi_group(self):
        ...
        for spin in [self.roi_xmin_spin, self.roi_xmax_spin]:
            ...
            spin.setEnabled(False) # 默认禁用
        
        self.btn_select_roi = QPushButton("鼠标框选范围")
        self.btn_clear_roi = QPushButton("清除范围")
        ...
        layout.addWidget(self.roi_xmin_spin, 1, 1)
        layout.addWidget(QLabel("X-Max:"), 1, 2)
        layout.addWidget(self.roi_xmax_spin, 1, 3)
    ...
    def _create_buck_designer_tab(self):
        ...
        ### MODIFICATION START: Add send TF button ###
        ...
        main_layout.addWidget(self.btn_send_buck_tf) ### Add to layout
    ...
    def _create_pfc_designer_tab(self):
        ...
        main_layout.addWidget(self.btn_send_pfc_tf) ### Add to layout
        main_layout.addStretch()
        return widget
    ...
    ### MODIFICATION START: Reorganized analysis group ###
    def _create_analysis_group(self):
        ...
        self.transient_param_widget = self._create_transient_param_widget()
        self.bode_param_widget = self._create_bode_param_widget() 
        self.tf_bode_param_widget = self._create_tf_bode_param_widget()
        self.fft_param_widget = self._create_fft_param_widget()
        self.pll_param_widget = self._create_pll_param_widget()
        self.adc_param_widget = self._create_adc_param_widget()
        self.fitting_param_widget = self._create_fitting_param_widget()
        
        self.param_stack.addWidget(self.transient_param_widget)
        self.param_stack.addWidget(self.bode_param_widget)
        self.param_stack.addWidget(self.tf_bode_param_widget)
        self.param_stack.addWidget(self.fft_param_widget)
        self.param_stack.addWidget(self.pll_param_widget)
        self.param_stack.addWidget(self.adc_param_widget)
        self.param_stack.addWidget(self.fitting_param_widget)
    ...
    def on_analysis_type_changed(self, index):
        """Handle side-effects of changing analysis type"""
    ...
    def _create_fitting_param_widget(self):
        ...
        self.fitting_method_combo.addItems([
            "线性拟合 (Linear)",
            "多项式拟合 (Polynomial)",
            "趋势振荡信号拟合 (Trending Oscillation)",
            "包络线分析 (Envelope Analysis)"
        ])
        ...
        self.fitting_poly_degree = QSpinBox()
        self.fitting_poly_degree.setRange(2, 10); self.fitting_poly_degree.setValue(2)
        ...
        self.fitting_params_stack.addWidget(QWidget()) # 0: Linear
        self.fitting_params_stack.addWidget(poly_widget) # 1: Polynomial
        self.fitting_params_stack.addWidget(QWidget()) # 2: Trending Oscillation
        self.fitting_params_stack.addWidget(env_widget) # 3: Envelope

        def switch_fitter_params(index):
            self.fitting_params_stack.setCurrentIndex(
                {0: 0, 1: 1, 2: 2, 3: 3}.get(index, 0)
            )
        self.fitting_method_combo.currentIndexChanged.connect(switch_fitter_params)
        
        layout.addRow(self.fitting_params_stack)
        switch_fitter_params(0)
    ...
    def _create_transient_param_widget(self):
        ...
        self.transient_method_combo = QComboBox()
        self.transient_method_combo.addItems(["带宽法", "RMSE", "标准差"]); self.transient_method_combo.setCurrentText("带宽法")
        layout.addRow("恢复时间判定方法:", self.transient_method_combo)
        
        self.transient_band_spin = QDoubleSpinBox()
        self.transient_band_spin.setRange(0.1, 50.0); self.transient_band_spin.setDecimals(1); self.transient_band_spin.setValue(1.0)
        self.transient_band_label = QLabel("稳态容限带宽(±%):")
        layout.addRow(self.transient_band_label, self.transient_band_spin)
        
        self.transient_window_combo = QComboBox()
        self.transient_window_combo.addItems(["10", "20", "50", "100", "200", "500", "1000"]); self.transient_window_combo.setCurrentText("100")
        ...
        self.transient_thresh_spin = QDoubleSpinBox()
        self.transient_thresh_spin.setRange(0.000001, 1.0); self.transient_thresh_spin.setDecimals(6); self.transient_thresh_spin.setValue(0.0002)
    ...
    def _create_bode_param_widget(self):
        widget = QWidget()
        layout = QFormLayout(widget)
        info_label = QLabel("文件需包含频率(Hz)、增益(dB)和相位(deg)列。")
    ...
    def _create_fft_param_widget(self):
        widget = QWidget()
        layout = QFormLayout(widget)
        self.fft_auto_fs_check = QCheckBox("根据时间列自动检测采样率")
        self.fft_auto_fs_check.setChecked(True)
        layout.addRow(self.fft_auto_fs_check)
        self.fft_fs_spin = QDoubleSpinBox()
        self.fft_fs_spin.setRange(1, 1e12); self.fft_fs_spin.setDecimals(2); self.fft_fs_spin.setValue(1000000.00)
    ...
    def _create_pll_param_widget(self):
        ...
        self.pll_method_combo = QComboBox(); self.pll_method_combo.addItems(["带宽法", "RMSE", "标准差"]); self.pll_method_combo.setCurrentText("带宽法")
        ...
        self.pll_window_combo = QComboBox()
        self.pll_window_combo.addItems(["10", "20", "50", "100", "200", "500", "1000"]); self.pll_window_combo.setCurrentText("100")
    ...
    def _create_action_buttons_group(self):
        ...
        export_layout.addWidget(self.btn_save_fig, 0, 1)
        
        layout.addWidget(self.btn_analyze)
        h_layout = QHBoxLayout()
        h_layout.addWidget(self.btn_show_osc)
        h_layout.addWidget(self.btn_clear_plot)
    ...
    def _create_right_panel(self):
        ...
        self.bode_tracker_check = QCheckBox("Bode图跟踪")
        self.bode_tracker_check.setEnabled(False)
        ...
        canvas_layout.addWidget(self.axis_control_group)
    ...
    def _create_axis_control_group(self):
        ...
        self.y_min_spin=QDoubleSpinBox(); layout.addWidget(QLabel("Y-Min:"),0,4); layout.addWidget(self.y_min_spin,0,5)
        self.y_max_spin=QDoubleSpinBox(); layout.addWidget(QLabel("Y-Max:"),0,6); layout.addWidget(self.y_max_spin,0,7)
        ...
        self.btn_reset_axis_limits=QPushButton("重置视图")
        
        layout.addWidget(self.btn_apply_axis_limits,0,8)
        layout.addWidget(self.btn_reset_axis_limits,0,9)
    ...
    def _create_cursor_group(self):
        ...
        self.cursor_stats_table = QTableWidget()
        self.cursor_stats_table.setEditTriggers(QTableWidget.NoEditTriggers)
    ...
    def _apply_styles(self):
        ...
        self.setFont(QFont("Microsoft YaHei",10));QApplication.instance().setStyleSheet("""
            ...
            QGroupBox::title{subcontrol-origin:margin;subcontrol-position:top left;padding:0 5px;color:#005A9C;}
